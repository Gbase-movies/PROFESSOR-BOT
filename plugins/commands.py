    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        ...
        if message.command[1] != "subscribe":
            try:
                ...
                btn.append([InlineKeyboardButton(" 🔄 JARIBU TENA ", callback_data=f"{pre}#{file_id}")])
            except (IndexError, ValueError):
                btn.append([InlineKeyboardButton(" 🔄 JARIBU TENA", url=f"https://t.me/{temp.U_NAME}?start={message.command[1]}")])
